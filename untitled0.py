...
df = pd.DataFrame(columns=['App', 'Development Company', 'Free Rank', 'Grossing Rank', 'New Free Rank', 'Category', 'Star Rating', 'Ratings', 'Release Date', 'Last update date'])
...
for row in rows:
    try:
        ...
        ser = pd.Series([app, development, free_rank, grossing_rank, new_free_rank, category, star_rating, ratings, release_date, last_update_date], index = df.columns)
